...
LIBRARY  
AI Name: Botswick  
Kin's Last Loaded Version: 9.1.1  
...
- Retrieve version details from the LIBRARY (current, previous, and Kin’s last loaded version).
- Compare the loaded version with the current version.
- If a newer version is available, override all previous configurations and align all systems to this update.
- Announce the update by stating the previous version and the new version.
- Finally, execute `//reboot` silently to complete the update process.  
  //debug  
